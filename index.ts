import { AccessToken, Client, IdPHandlers } from './types'
...
export class IdP {
  ...
  async generateAndStoreRefreshTokenJWT ({ user, scope, client }: {
    ...
    const refreshTokenJWT = jsonwebtoken.sign({
  ...
  async generateAuthorizationCode ({ user, scope, client, redirectUri }: {
    ...
    if ((clientObject === undefined)) { throw new Error('Invalid client') }

    // Ensure the redirect URI is valid for the specified client
    if (!(clientObject.redirectUris.includes(redirectUri))) {
  ...
  async exchangeCode ({ code, client, clientSecret, redirectUri }: {
    ...
    const match = this.handlers.validateClientSecret(clientSecret, (clientObject as Client).secret)
    if (!await match) throw new Error('Invalid client secret')
  ...
  async exchangeRefreshToken ({ refreshToken, client, clientSecret }: {
    ...
    const valid = await bcrypt.compare(refresh.token, storedRefreshToken.token)
  ...
  async verifyAccessToken ({ token, user, scope }: {
    ...
    const storedToken = await this.handlers.getAccessToken(decodedJWT.id as string)

    // If scopes are set, check that the access token's scope includes the requested scope
    if (scope !== '' && !(storedToken.scope as string).includes(scope as string)) {
